    ...
    for (unsigned int v = 0; v < dag.size(); v++)
    {
        ...
        for (int neighbor : dag[v])
        {
            ...
            for (int c1 : candidates)
            {
                ...
                for (int c2 : candidate_sets[neighbor])
                {
                    if (candidate_edges[c1].find(c2) != candidate_edges[c1].end())
                    {
                        unconnected = false;
                        break;
                    }
    ...
    vector<set<int>> &parents)
{
    ...
    for (int current : frontier)
    {
        ...
        {
            bool parents_not_mapped = true;
            for (int parent : parents[current])
            {
                if (seen.find(parent) == seen.end())
                {
                    parents_not_mapped = false;
                    break;
                }
            }
            if (parents_not_mapped)
            {
                continue;
            }
        }
        ...
        {
            if (new_mapped.find(candidate) != new_mapped.end())
            {
                continue;
            }
            ...
            // For Data Neighbors of Candidate
            for (int neighbor : candidate_edges[candidate])
            {
                // Find all Query Candidate Sets with the Data Neighbor
                for (int v : query_dag[current])
                {
                    if (seen.find(v) != seen.end())
                    {
                        continue;
                    }
                    // If there exists an edge between Current Vertex and Query Vertex, Search
                    if (candidate_sets[v].find(neighbor) != candidate_sets[v].end())
                    {
